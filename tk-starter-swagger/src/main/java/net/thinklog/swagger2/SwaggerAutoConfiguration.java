...
public class SwaggerAutoConfiguration implements BeanFactoryAware {
    ...
    @Bean
    @ConditionalOnMissingBean
    @ConditionalOnProperty(name = "tk.swagger.enabled", matchIfMissing = true)
    public List<Docket> createRestApi(SwaggerProperties swaggerProperties) {
        ...
        for (String groupName : swaggerProperties.getDocket().keySet()) {
            ...
            Docket docket = new Docket(DocumentationType.SWAGGER_2)
                    ...
                    //最新版的断言
                    .paths(s -> excludePath.stream().noneMatch(stringPredicate -> stringPredicate.test(s))
                            || basePath.stream().allMatch(stringPredicate -> stringPredicate.test(s))
                    )
